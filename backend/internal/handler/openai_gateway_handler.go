...
// ChatCompletions handles OpenAI Chat Completions endpoint
...
func (h *OpenAIGatewayHandler) ChatCompletions(c *gin.Context) {
	...
	for {
		log.Printf("[OpenAI Chat] Selecting account: groupID=%v model=%s", apiKey.GroupID, reqModel)
		selection, err := h.gatewayService.SelectAccountWithLoadAwareness(c.Request.Context(), apiKey.GroupID, sessionHash, reqModel, failedAccountIDs)
		if err != nil {
			log.Printf("[OpenAI Chat] SelectAccount failed: %v", err)
			if len(failedAccountIDs) == 0 {
				h.handleStreamingAwareError(c, http.StatusServiceUnavailable, "api_error", "No available accounts: "+err.Error(), streamStarted)
				return
			}
		...
		account := selection.Account
		log.Printf("[OpenAI Chat] Selected account: id=%d name=%s", account.ID, account.Name)
		setOpsSelectedAccount(c, account.ID)
		...
		if err != nil {
			...
			log.Printf("Account %d: Forward chat request failed: %v", account.ID, err)
			return
		}
