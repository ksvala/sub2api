...
func initializeApplication(buildInfo handler.BuildInfo) (*Application, error) {
	...
	inviteRepository := repository.NewInviteRepository(client)
	inviteLogRepository := repository.NewInviteLogRepository(client)
	inviteService := service.NewInviteService(client, inviteRepository, inviteLogRepository, userRepository, redeemCodeRepository, settingService, billingCacheService, apiKeyAuthCacheInvalidator)
	authService := service.NewAuthService(userRepository, redeemCodeRepository, refreshTokenCache, configConfig, settingService, emailService, turnstileService, emailQueueService, promoService, inviteService)
	...
	subscriptionReminderService := service.NewSubscriptionReminderService(userRepository, userSubscriptionRepository, redisClient, emailQueueService)
	subscriptionHandler := handler.NewSubscriptionHandler(subscriptionService, subscriptionReminderService)
	inviteHandler := handler.NewInviteHandler(inviteService)
	planRepository := repository.NewPlanRepository(client)
	...
	adminService := service.NewAdminService(userRepository, groupRepository, accountRepository, proxyRepository, apiKeyRepository, redeemCodeRepository, inviteService, userGroupRateRepository, billingCacheService, proxyExitInfoProber, proxyLatencyCache, apiKeyAuthCacheInvalidator)
	...
	adminActionLogRepository := repository.NewAdminActionLogRepository(client)
	adminActionLogService := service.NewAdminActionLogService(adminActionLogRepository)
	adminPlanHandler := admin.NewPlanHandler(planService, adminActionLogService)
	adminAnnouncementHandler := admin.NewAnnouncementHandler(announcementService)
	uploadService := service.NewUploadService()
	uploadHandler := admin.NewUploadHandler(uploadService, adminActionLogService)
	...
	settingHandler := admin.NewSettingHandler(settingService, emailService, turnstileService, opsService, adminActionLogService)
	...
	adminInviteHandler := admin.NewInviteHandler(inviteService, adminActionLogService)
	...
	adminHandlers := handler.ProvideAdminHandlers(dashboardHandler, adminUserHandler, groupHandler, accountHandler, oAuthHandler, openAIOAuthHandler, geminiOAuthHandler, antigravityOAuthHandler, proxyHandler, adminRedeemHandler, promoHandler, adminPlanHandler, adminAnnouncementHandler, uploadHandler, settingHandler, opsHandler, systemHandler, adminSubscriptionHandler, adminUsageHandler, userAttributeHandler, adminInviteHandler, errorPassthroughHandler)
	...
	handlers := handler.ProvideHandlers(authHandler, userHandler, apiKeyHandler, usageHandler, redeemHandler, subscriptionHandler, inviteHandler, planHandler, announcementHandler, adminHandlers, gatewayHandler, openAIGatewayHandler, handlerSettingHandler, totpHandler)
